      ...
      <div className="mx-auto max-w-[88rem] px-5 sm:px-8">
        <div className="grid gap-16 lg:grid-cols-12 lg:items-center lg:gap-20">
            ...
            <SectionHeading
              ...
              title={
                <>
                  The Home Movers
                  <br />
                  <span className="italic text-ember-600">Magazine.</span>
                </>
              }
          ...
          <div className="lg:col-span-6">
            <Reveal
              delay={140}
              className="rounded-3xl border border-bone-300/80 bg-bone-50 p-8 shadow-[0_30px_60px_-40px_rgba(6,8,12,0.35)] sm:p-10"
            >
              <p className="flex items-center justify-between text-[10.5px] font-semibold uppercase tracking-[0.22em] text-ink-400">
                <span>Inside this issue</span>
                <span>No. 04</span>
              </p>

              <ul className="mt-8 border-t border-bone-300/80">
                {FEATURES.map((feature, index) => (
                  <li
                    key={feature.title}
                    className="flex items-baseline gap-5 border-b border-bone-300/80 py-5"
                  >
                    <span className="font-display w-6 shrink-0 text-sm text-bone-500 tabular-nums">
                      {String(index + 1).padStart(2, "0")}
                    </span>
                    <div className="min-w-0 flex-1">
                      <p className="text-[10.5px] font-semibold uppercase tracking-[0.16em] text-ember-600">
                        {feature.kicker}
                      </p>
                      <h3 className="font-display mt-1.5 text-xl text-ink-900">
                        {feature.title}
                      </h3>
